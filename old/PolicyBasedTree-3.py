...
class SimulationPolicy(nn.Module):
    def __init__(self,  env, hdim=100, maxBranchingFactor=3, nlayers=1):
        ...
        self.lstm = nn.LSTM(input_size=self.stateSize, hidden_size=self.hdim, num_layers=nlayers)
        self.hiddenToAction = nn.Linear(self.hdim, self.actionSize)
        # Layer to sample branching factor
        self.intSamplingLayer = nn.Linear(self.hdim, self.maxBranchingFactor + 1)
    ...
    def sample(self, state, temperature=2,branching_temperature=1,verbose=False):
        ...
        output = self.hiddenToAction(lstm_out[0,0,:])
        output = output.unsqueeze(dim=0)
        soft_action = F.softmax(output, dim=1)
        m = nn.LogSoftmax(dim=1)
        output = m(output)
        # Use Gumbel-Softmax 
        gumbeled_action = gumbel_softmax(output, temperature)

        b_ann = self.intSamplingLayer(lstm_out[0,0,:])
        b_ann = b_ann.unsqueeze(dim=0)
        ...
        return gumbeled_action, soft_action, branchingRateSample, self.hidden
...
class Node(object):
    ...
    def __init__(self, parent_node, state, action, sampledAction, hidden, policyHidden):
...
class Tree(object):
    ...
    def __init__(self, initialState, forwardModel, simPolicy, valueF, env, maxDepth=5, branchingFactor=3):
        ...
        self.simPolicy.reInitialize(1)
        parent = Node(None, initialState, None, None, self.forwardModel.hidden, self.simPolicy.hidden)
    ...
    def grow(self,node,d,b,verbose=False):
        ...
        while (i < b.data).all():
            ...
            self.simPolicy.setHiddenState(node.policyHidden)
            hard_action, soft_a_s, new_branching_breadth, current_policy_hidden = self.simPolicy.sample(node.state)
            ...
            childNode = Node(node, current_state, [soft_a_s], [hard_action], current_hidden, current_policy_hidden)
...
def main():
    ...
    exampleEnv = generateTask(0,0,0,5,8) # This takes about 10 sec to train & solve on my comp
    ...
    if runTraining:
        ...
        tree = Tree(s_0, ForwardModel, SimPolicy, greedy_valueF, exampleEnv, maxDepth=5, branchingFactor=2)
