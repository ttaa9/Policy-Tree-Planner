...
class SimulationPolicy(nn.Module):
    ...
    def trainSad(self, taskEnv, forwardModel, printActions=False, maxDepth=5, treeBreadth=2, eta_lr=0.0005):
...
#       cenv = generateTask(p[0],p[1],orien,g[0],g[1])
        ...
        for i in range(0,1000):
            ...
            if i % 50 == 0: 
                ...
                if printActions:
                    print( "\n".join([ ",".join([str(qq) for qq in a[0].data.numpy()]) for a in tree.getBestPlan()[1]] ) )
...
#         print('Getting leaves')
    ...
    def getLossFromLeaves(self, alpha=0.5, lambda_h=-0.025, useHolder=False, holderp=-5.0):
        ...
        for i, leaf in enumerate(self.leaves):
            totalInverseValue += -self.valueF( leaf.state ).pow( holderp )  
        ...
        totalLoss = alpha*(totalInverseValue / nleaves).pow( 1.0 / holderp) + lambda_h * totalEntropy
        return totalLoss  #torch.min(totalLosses)
...
def main():
    ...
    # Run training
    if runTraining:
        SimPolicy.trainSad(exampleEnv, ForwardModel,printActions=True,maxDepth=3,treeBreadth=2) 
        # 
        s_0 = torch.unsqueeze(avar(torch.FloatTensor(exampleEnv.getStateRep())), dim=0)
        tree = Tree(s_0, ForwardModel, SimPolicy, greedy_cont_valueF, exampleEnv, 2, 3)
